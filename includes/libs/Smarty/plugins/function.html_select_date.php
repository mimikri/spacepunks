 ...
 */
function smarty_function_html_select_date($params, Smarty_Internal_Template $template): string
{
    ...
    if (isset($time) && is_array($time)) {
        ...
        $template->_checkPlugins(
            [['function' => 'smarty_make_timestamp', 'file'     => SMARTY_PLUGINS_DIR . 'shared.make_timestamp.php']]
        );
    ...
    foreach (['start', 'end'] as $key) {
        ...
        if ($t === null) {
            ...
            ${$key} = (int)($_current_year + (int)trim(substr((string) $t, 1)));
        } elseif ($t[ 0 ] === '-') {
            ${$key} = (int)($_current_year - (int)trim(substr((string) $t, 1)));
        } else {
            ${$key} = (int)${$key};
