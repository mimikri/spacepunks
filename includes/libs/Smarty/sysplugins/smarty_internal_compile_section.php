...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
     ...
     * @var array
     */
    public $nameProperties = ['first', 'last', 'index', 'iteration', 'show', 'total', 'rownum', 'index_prev', 'index_next', 'loop'];
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler): string
    {
        ...
        $incFor = [];
        $cmpFor = [];
        $propValue = ['index'     => "{$sectionVar}->value['index']", 'show' => 'true', 'step' => 1, 'iteration' => "{$local}iteration"];
        ...
        if (isset($_attr[ 'properties' ]) && preg_match_all("/['](.*?)[']/", (string) $_attr[ 'properties' ], $match)) {
        ...
        if (!isset($propValue[ 'start' ])) {
            ...
            $start_code =
                [1  => "{$propValue['start']} < 0 ? ", 2 => 'max(', 3 => "{$propValue['step']} > 0 ? ", 4 => '0', 5  => ' : ', 6 => '-1', 7 => ', ', 8 => "{$propValue['start']} + {$propValue['loop']}", 10 => ')', 11 => ' : ', 12 => 'min(', 13 => $propValue[ 'start' ], 14 => ', ', 15 => "{$propValue['step']} > 0 ? ", 16 => $propValue[ 'loop' ], 17 => ' : ', 18 => $propType[ 'loop' ] === 0 ? $propValue[ 'loop' ] - 1 : "{$propValue['loop']} - 1", 19 => ')'];
            if ($propType[ 'step' ] === 0) {
        ...
        if (!isset($_attr[ 'start' ]) && !isset($_attr[ 'step' ]) && !isset($_attr[ 'max' ])) {
            ...
            if ($propType[ 'total' ] === 0) {
                ...
                $total_code = [1  => 'min(', 2 => 'ceil(', 3 => '(', 4 => "{$propValue['step']} > 0 ? ", 5  => $propValue[ 'loop' ], 6 => ' - ', 7 => $propValue[ 'start' ], 8 => ' : ', 9  => $propValue[ 'start' ], 10 => '+ 1', 11 => ')', 12 => '/ ', 13 => 'abs(', 14 => $propValue[ 'step' ], 15 => ')', 16 => ')', 17 => ", {$propValue['max']})"];
                if (!isset($propValue[ 'max' ])) {
