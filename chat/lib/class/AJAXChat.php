...
class AJAXChat {
	...
	function insertCustomMessage($userID, $userName, $userRole, string $channelID, $text, $ip=null, $mode=0): void {
	...
	function getXMLMessages(): string {
		return match ($this->getView()) {
   ...
   ['expires' => time() + (60 * 60 * 24 * $this->getConfig('sessionCookieLifeTime')), 'path' => (string) $this->getConfig('sessionCookiePath'), 'domain' => $this->getConfig('sessionCookieDomain') ?? $_SERVER['HTTP_HOST'], 'secure' => isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off', 'httponly' => true]
	...
	function removeUnsafeCharacters($str): string|array|null {
	...
	function subString($str, $start=0, $length=null, $encoding='UTF-8'): string|false {
		return AJAXChatString::subString($str, $start, $length, $encoding);
	}
	
	function stringLength($str, $encoding='UTF-8'): int|false {
	...
	function encodeEntities($str, $encoding='UTF-8', $convmap=null): string {
		return AJAXChatEncoding::encodeEntities($str, $encoding, $convmap);
	}

	function decodeEntities($str, $encoding='UTF-8', $htmlEntitiesMap=null): string {
